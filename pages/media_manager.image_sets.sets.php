...
use FriendsOfRedaxo\ImageSets\Config\PresetStore;
use FriendsOfRedaxo\ImageSets\Config\SetBuilder;
...
$csrf = rex_csrf_token::factory('image_sets_sets');
...
if ($func !== '' && !$csrf->isValid()) {
    $message = rex_view::error($t('image_sets_msg_csrf'));
    $func = '';
}
if ($func === 'save') {
    ...
    if ($errors === []) {
        ...
        $message = rex_view::success($t('image_sets_msg_saved', $name));
    } else {
    ...
    if (PresetStore::get($name) !== null) {
        ...
        $message = rex_view::success($t('image_sets_msg_deleted', $name));
    }
} elseif ($func === 'toggle') {
    $name = rex_request('name', 'string', '');
    ...
    if ($preset !== null) {
        ...
        $message = rex_view::success($t('image_sets_msg_status', $name, $t(!$preset['active'] ? 'image_sets_status_active' : 'image_sets_status_inactive')));
    }
} elseif ($func === 'clear_cache') {
    $name = rex_request('name', 'string', '');
    $message = rex_view::success($t('image_sets_msg_cache_cleared', (string) PresetStore::clearCache($name), $name));
}
echo $message;
...
$list = '<p>' . $t('image_sets_sets_intro') . '</p>';
$list .= '<div class="table-responsive"><table class="table table-striped table-hover"><thead><tr>'
    . '<th>' . $t('image_sets_col_name') . '</th><th>' . $t('image_sets_col_ratio') . '</th><th>' . $t('image_sets_col_mode') . '</th><th>' . $t('image_sets_col_widths') . '</th><th>' . $t('image_sets_col_default') . '</th><th>' . $t('image_sets_col_source') . '</th><th>' . $t('image_sets_col_status') . '</th><th>' . $t('image_sets_col_cache') . '</th><th class="rex-table-action">&nbsp;</th>'
    . '</tr></thead><tbody>';
foreach ($rows as $row) {
    $name = $row['name'];
    $cfg = $row['config'];
    ...
    $list .= '<td>' . ($row['source'] === 'code' ? '<span class="label label-default">' . $t('image_sets_source_code') . '</span>' : '<span class="label label-primary">' . $t('image_sets_source_builder') . '</span>') . '</td>';
    $list .= '<td>' . ($row['active'] ? '<span class="label label-success">' . $t('image_sets_status_active') . '</span>' : '<span class="label label-warning">' . $t('image_sets_status_inactive') . '</span>') . '</td>';
    $list .= '<td>' . ($cache['files'] > 0 ? $e($t('image_sets_cache_files', (string) $cache['files'], rex_formatter::bytes($cache['bytes']))) : '<span class="text-muted">' . $t('image_sets_cache_none') . '</span>') . '</td>';
    $list .= '<td class="rex-table-action" style="white-space:nowrap">';
    $list .= '<a class="btn btn-xs btn-default" href="' . rex_url::currentBackendPage(['preview_preset' => $name]) . '#is-preview"><i class="rex-icon fa-eye"></i> ' . $t('image_sets_action_test') . '</a> ';
    if ($row['source'] === 'builder') {
        $list .= '<a class="btn btn-xs btn-default" href="' . rex_url::currentBackendPage(['edit' => $name]) . '#is-form"><i class="rex-icon rex-icon-edit"></i> ' . $t('image_sets_action_edit') . '</a> ';
        $list .= '<a class="btn btn-xs btn-default" href="' . $link('toggle', ['name' => $name]) . '">' . $t($row['active'] ? 'image_sets_action_deactivate' : 'image_sets_action_activate') . '</a> ';
        if ($inUse) {
            $list .= '<span class="btn btn-xs btn-default disabled" title="' . $e($t('image_sets_delete_blocked')) . '"><i class="rex-icon rex-icon-delete"></i></span> ';
        } else {
            $list .= '<a class="btn btn-xs btn-delete" href="' . $link('delete', ['name' => $name]) . '" data-confirm="' . $e($t('image_sets_confirm_delete', $name)) . '"><i class="rex-icon rex-icon-delete"></i> ' . $t('image_sets_action_delete') . '</a> ';
        }
    }
    if ($cache['files'] > 0) {
        $list .= '<a class="btn btn-xs btn-default" href="' . $link('clear_cache', ['name' => $name]) . '" data-confirm="' . $e($t('image_sets_confirm_clear_cache')) . '"><i class="rex-icon fa-eraser"></i> ' . $t('image_sets_action_clear_cache') . '</a>';
    }
    $list .= '</td></tr>';
}
$list .= '</tbody></table></div>';
$list .= '<a class="btn btn-primary" href="' . rex_url::currentBackendPage(['new' => 1]) . '#is-form"><i class="rex-icon rex-icon-add"></i> ' . $t('image_sets_action_new') . '</a>';

$fragment = new rex_fragment();
$fragment->setVar('title', $t('image_sets_sets_list_title'), false);
...
$containers = [
    'uk-container-xsmall' => $t('image_sets_container_xsmall'), 'uk-container-small' => $t('image_sets_container_small'), 'uk-container' => $t('image_sets_container_default'),
    'uk-container-large' => $t('image_sets_container_large'), 'uk-container-xlarge' => $t('image_sets_container_xlarge'), 'expand' => $t('image_sets_container_expand'),
];
...
$form .= $field($t('image_sets_field_name'), '<input class="form-control" type="text" name="name" value="' . $e($values['name']) . '" pattern="[a-z][a-z0-9_]{1,60}" required placeholder="teaser_3col">', $t('image_sets_field_name_notice'));
...
$ratioSelect .= '<option value="custom"' . ($values['ratio'] === 'custom' ? ' selected' : '') . '>' . $t('image_sets_ratio_custom') . '</option></select>';
...
$form .= $field($t('image_sets_field_ratio'), $ratioSelect, $t('image_sets_field_ratio_custom_notice'));
$form .= $field($t('image_sets_field_mode'), '<select class="form-control" name="preset[mode]" id="is-f-mode"><option value="focuspoint"' . ($values['mode'] === 'focuspoint' ? ' selected' : '') . '>' . $t('image_sets_mode_focuspoint') . '</option><option value="resize"' . ($values['mode'] === 'resize' ? ' selected' : '') . '>' . $t('image_sets_mode_resize') . '</option></select>');
$form .= $field($t('image_sets_field_widths'), '<input class="form-control" type="text" name="preset[widths]" id="is-f-widths" value="' . $e($values['widths']) . '" required>', $t('image_sets_field_widths_notice', (string) PresetStore::MIN_WIDTH, (string) PresetStore::MAX_WIDTH, (string) PresetStore::MAX_STEPS));
$form .= $field($t('image_sets_field_default'), '<input class="form-control" type="number" name="preset[default_width]" id="is-f-default" value="' . $e($values['default_width']) . '" min="' . PresetStore::MIN_WIDTH . '" max="' . PresetStore::MAX_WIDTH . '">', $t('image_sets_field_default_notice'));
$form .= $field($t('image_sets_field_note'), '<input class="form-control" type="text" name="preset[note]" value="' . $e($values['note']) . '" maxlength="200">', $t('image_sets_field_note_notice'));
$form .= '<div class="checkbox"><label><input type="checkbox" name="preset[active]" value="1"' . ($values['active'] ? ' checked' : '') . '> ' . $t('image_sets_field_active') . '</label></div>';
$form .= '<p style="margin-top:12px"><button class="btn btn-save" type="submit">' . $t('image_sets_btn_save') . '</button> ' . ($editing !== null ? '<a class="btn btn-default" href="' . rex_url::currentBackendPage() . '">' . $t('image_sets_btn_cancel') . '</a>' : '') . '</p>';
$form .= '</div><div class="col-md-6">';
// Assistent
$form .= '<div class="panel panel-default"><div class="panel-heading"><strong>' . $t('image_sets_assistant_title') . '</strong></div><div class="panel-body" id="is-assistant">';
$form .= '<p class="help-block">' . $t('image_sets_assistant_intro') . '</p>';
$containerSelect = '<select class="form-control" id="is-a-container">';
foreach ($containers as $value => $label) {
    $containerSelect .= '<option value="' . $e($value) . '" data-px="' . (int) (SetBuilder::CONTAINERS[$value] ?? 1200) . '"' . ($value === 'uk-container' ? ' selected' : '') . '>' . $e($label) . '</option>';
}
$containerSelect .= '</select>';
$form .= '<div class="row"><div class="col-sm-12">' . $field($t('image_sets_assistant_container'), $containerSelect) . '</div>';
$form .= '<div class="col-sm-4">' . $field($t('image_sets_assistant_cols_desktop'), '<input class="form-control" type="number" id="is-a-cd" min="1" max="6" value="3">') . '</div>';
$form .= '<div class="col-sm-4">' . $field($t('image_sets_assistant_cols_tablet'), '<input class="form-control" type="number" id="is-a-ct" min="1" max="4" value="2">') . '</div>';
$form .= '<div class="col-sm-4">' . $field($t('image_sets_assistant_cols_mobile'), '<input class="form-control" type="number" id="is-a-cm" min="1" max="2" value="1">') . '</div>';
$form .= '<div class="col-sm-6">' . $field($t('image_sets_assistant_fraction'), '<input class="form-control" type="number" id="is-a-fraction" min="5" max="100" value="100">', $t('image_sets_assistant_fraction_notice')) . '</div>';
$form .= '<div class="col-sm-6">' . $field($t('image_sets_assistant_source_max'), '<input class="form-control" type="number" id="is-a-source" min="100" max="' . PresetStore::MAX_WIDTH . '" step="100" value="' . (int) ($suggest['steps'][3]['x2'] > 0 ? min(2400, max(800, SetBuilder::roundUp($stats['max'] ?: 2400))) : 2400) . '">', $t('image_sets_assistant_source_max_notice')) . '</div>';
$form .= '<div class="col-sm-6">' . $field($t('image_sets_assistant_bp_tablet'), '<input class="form-control" type="number" id="is-a-bpt" min="320" max="1600" value="640">') . '</div>';
$form .= '<div class="col-sm-6">' . $field($t('image_sets_assistant_bp_desktop'), '<input class="form-control" type="number" id="is-a-bpd" min="480" max="2400" value="1200">') . '</div>';
$form .= '<div class="col-sm-12"><div class="checkbox"><label><input type="checkbox" id="is-a-retina" checked> ' . $t('image_sets_assistant_retina') . '</label></div></div></div>';
$form .= '<h5>' . $t('image_sets_assistant_steps') . '</h5><table class="table table-condensed" id="is-a-steps"><thead><tr><th></th><th>' . $t('image_sets_assistant_css') . '</th><th>' . $t('image_sets_assistant_1x') . '</th><th>' . $t('image_sets_assistant_2x') . '</th></tr></thead><tbody>';
foreach (['phone', 'mobile', 'tablet', 'desktop'] as $key) {
    $form .= '<tr data-step="' . $key . '"><td>' . $t('image_sets_step_' . $key) . '</td><td class="is-css">–</td><td class="is-x1">–</td><td class="is-x2">–</td></tr>';
}
$form .= '</tbody></table>';
$form .= '<p><strong>' . $t('image_sets_assistant_result') . ':</strong> <code id="is-a-result">' . $e(implode(', ', $suggest['widths'])) . '</code> · ' . $t('image_sets_col_default') . ' <code id="is-a-default">' . (int) $suggest['default_width'] . '</code></p>';
$form .= '<button class="btn btn-default" type="button" id="is-a-apply"><i class="rex-icon fa-magic"></i> ' . $t('image_sets_assistant_apply') . '</button>';
$form .= '</div></div></div></div></form>';

$fragment = new rex_fragment();
$fragment->setVar('title', $editing !== null ? $t('image_sets_form_title_edit', $editName) : $t('image_sets_form_title_new'), false);
...
// 3. Nutzung & Bestand
// ---------------------------------------------------------------------
$analysis = '<p>' . $t('image_sets_analysis_intro') . '</p><div class="row"><div class="col-md-7">';
$analysis .= '<h4>' . $t('image_sets_analysis_usage_title') . '</h4>';
if ($usage === []) {
    $analysis .= '<p class="text-muted">' . $t('image_sets_analysis_usage_none') . '</p>';
} else {
    $analysis .= '<table class="table table-condensed table-striped"><thead><tr><th>' . $t('image_sets_col_name') . '</th><th>' . $t('image_sets_analysis_used_in') . '</th><th>' . $t('image_sets_analysis_widths_requested') . '</th></tr></thead><tbody>';
    foreach ($usage as $preset => $u) {
        $analysis .= '<tr><td><code>' . $e($preset) . '</code>' . (isset($registered[$preset]) ? '' : ' <span class="label label-danger">' . $t('image_sets_analysis_not_registered') . '</span>') . '</td><td>' . $e(implode(', ', $u['modules'])) . '</td><td>' . $e($u['widths'] !== [] ? implode(', ', $u['widths']) : '–') . '</td></tr>';
    }
    $analysis .= '</tbody></table>';
}
$analysis .= '</div><div class="col-md-5"><h4>' . $t('image_sets_analysis_media_title') . '</h4>';
$analysis .= '<p>' . $e($t('image_sets_analysis_media_total', (string) $stats['total'], (string) $stats['median'], (string) $stats['max'])) . '</p>';
$analysis .= '<table class="table table-condensed"><thead><tr><th>' . $t('image_sets_analysis_bucket') . '</th><th class="text-right">' . $t('image_sets_analysis_count') . '</th><th></th></tr></thead><tbody>';
...
foreach ($usage as $preset => $u) {
    if (!isset($registered[$preset])) {
        $hints[] = ['danger', $t('image_sets_analysis_hint_unregistered', $preset)];
        continue;
    }
    $widths = $registered[$preset]['widths'] ?? [];
    foreach ($u['widths'] as $requested) {
        if (!in_array($requested, $widths, true)) {
            $hints[] = ['info', $t('image_sets_analysis_hint_missing', $preset, (string) $requested, (string) MediaTypeRegistry::normalizeWidth($registered[$preset], $requested))];
        }
    }
    if ($u['widths'] !== []) {
        foreach ($widths as $w) {
            if (!in_array($w, $u['widths'], true)) {
                $hints[] = ['default', $t('image_sets_analysis_hint_unused', $preset, (string) $w)];
            }
        }
    }
}
foreach ($registered as $preset => $config) {
    foreach ($config['widths'] ?? [] as $w) {
        ...
        if ($share >= 80 && $stats['total'] > 0) {
            $hints[] = ['warning', $t('image_sets_analysis_hint_upscale', $preset, (string) $share, (string) $w)];
        }
    }
}
$analysis .= '<h4>' . $t('image_sets_analysis_hints') . '</h4>';
if ($hints === []) {
    $analysis .= '<p class="text-muted">' . $t('image_sets_analysis_hint_ok') . '</p>';
} else {
...
$fragment = new rex_fragment();
$fragment->setVar('title', $t('image_sets_analysis_title'), false);
...
$pf .= '<p>' . $t('image_sets_preview_intro') . '</p><div class="row">';
$pf .= '<div class="col-md-5">' . $field($t('image_sets_preview_media'), rex_var_media::getWidget(1, 'preview_media', $pFile)) . '</div>';
...
$presetSelect .= '</select>';
$pf .= '<div class="col-md-4">' . $field($t('image_sets_preview_preset'), $presetSelect) . '</div>';
$pMobile = rex_request('preview_mobile', 'string', '');
if ($pMobile !== '' && !isset($previewPresets[$pMobile])) {
    $pMobile = '';
}
$mobileSelect = '<select class="form-control" name="preview_mobile"><option value="">' . $t('image_sets_preview_mobile_none') . '</option>';
foreach ($previewPresets as $name => $config) {
...
$containerSelect .= '</select>';
$pf .= '<div class="col-md-3">' . $field($t('image_sets_assistant_container'), $containerSelect) . '</div></div><div class="row">';
$pf .= '<div class="col-md-4">' . $field($t('image_sets_preview_mobile'), $mobileSelect, $t('image_sets_preview_mobile_notice')) . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_cols_desktop'), '<input class="form-control" type="number" name="preview_columns" min="1" max="6" value="' . $pCols . '">') . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_cols_tablet'), '<input class="form-control" type="number" name="preview_columns_tablet" min="1" max="4" value="' . $pColsT . '">') . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_cols_mobile'), '<input class="form-control" type="number" name="preview_columns_mobile" min="1" max="2" value="' . $pColsM . '">') . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_fraction'), '<input class="form-control" type="number" name="preview_fraction" min="5" max="100" value="' . $pFraction . '">') . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_bp_tablet'), '<input class="form-control" type="number" name="preview_bp_tablet" min="320" max="1600" value="' . $pBpT . '">') . '</div>';
$pf .= '<div class="col-md-2">' . $field($t('image_sets_assistant_bp_desktop'), '<input class="form-control" type="number" name="preview_bp_desktop" min="480" max="2400" value="' . $pBpD . '">') . '</div>';
$pf .= '</div><button class="btn btn-primary" type="submit"><i class="rex-icon fa-eye"></i> ' . $t('image_sets_preview_show') . '</button></form>';
...
if ($doPreview && $pMedia !== null && $pPreset !== '') {
    ...
    $previewBody .= '<hr><h4>' . $t('image_sets_preview_markup') . '</h4><pre style="white-space:pre-wrap">' . $e($tag) . '</pre>';
    $previewBody .= '<p><span class="label label-default">width × height</span> ' . (int) $data['width'] . ' × ' . (int) $data['height'] . ' px &nbsp; <span class="label label-default">alt</span> ' . ($data['decorative'] ? '<em>' . $t('image_sets_preview_alt_decorative') . '</em>' : $e($data['alt'])) . '</p>';
    $previewBody .= '<h4>' . $t('image_sets_preview_variants') . '</h4><div class="table-responsive"><table class="table table-condensed table-striped"><thead><tr><th>' . $t('image_sets_preview_type') . '</th><th>' . $t('image_sets_preview_descriptor') . '</th><th>' . $t('image_sets_preview_file') . '</th><th>' . $t('image_sets_preview_size') . '</th><th>' . $t('image_sets_preview_status') . '</th></tr></thead><tbody>';
    $totalBytes = 0;
    $srcsetList = [];
    foreach ($entries as [$typeWidth, $descriptorWidth]) {
        ...
        $status = $error !== '' ? '<span class="label label-danger">' . $t('image_sets_preview_error') . '</span> ' . $e($error)
            : ($ok ? '<span class="label label-success">' . $t('image_sets_preview_ok') . '</span>' : '<span class="label label-warning">' . $e($t('image_sets_preview_deviation', (string) ($actualW - (int) $descriptorWidth))) . '</span>');
    ...
    $previewBody .= '</tbody></table></div><p class="text-muted">' . $e($t('image_sets_preview_total', rex_formatter::bytes($totalBytes))) . '</p>';
    $srcsetJson = json_encode($srcsetList, JSON_UNESCAPED_SLASHES) ?: '[]';
    $sizesJson = json_encode((string) ($data['sizes'] ?? ''), JSON_UNESCAPED_SLASHES) ?: '""';

    // Simulation
    $previewBody .= '<div class="row"><div class="col-md-7"><h4>' . $t('image_sets_preview_sim_title') . '</h4><p class="help-block">' . $t('image_sets_preview_sim_intro') . ' <code>sizes="' . $e((string) ($data['sizes'] ?? '')) . '"</code></p>';
    $previewBody .= '<table class="table table-condensed" id="is-sim"><thead><tr><th>' . $t('image_sets_preview_sim_viewport') . '</th><th>DPR</th><th>' . $t('image_sets_preview_sim_slot') . '</th><th>' . $t('image_sets_preview_sim_needed') . '</th><th>' . $t('image_sets_preview_sim_chosen') . '</th></tr></thead><tbody></tbody></table>';
    $previewBody .= '<div class="form-group"><label>' . $t('image_sets_preview_sim_custom') . ': <span id="is-sim-custom-val">1024</span> px</label><input type="range" id="is-sim-custom" min="320" max="2560" step="10" value="1024" style="width:100%"></div>';
    $previewBody .= '<div id="is-sim-custom-out" class="well well-sm"></div></div>';
    $previewBody .= '<div class="col-md-5"><h4>' . $t('image_sets_preview_live_title') . '</h4><p class="help-block">' . $t('image_sets_preview_live_intro') . '</p>';
    $previewBody .= '<div style="border:1px solid #ddd;padding:8px;background:#fff">' . $tag . '</div>';
    $previewBody .= '<dl class="dl-horizontal" style="margin-top:10px"><dt>' . $t('image_sets_preview_live_src') . '</dt><dd id="is-live-src">–</dd><dt>' . $t('image_sets_preview_live_width') . '</dt><dd id="is-live-nw">–</dd><dt>' . $t('image_sets_preview_live_container') . '</dt><dd id="is-live-cw">–</dd><dt>Status</dt><dd id="is-live-verdict">' . $t('image_sets_preview_loading') . '</dd></dl></div></div>';
} elseif ($doPreview && $pMedia === null) {
    $previewBody .= rex_view::warning($t('image_sets_preview_no_media'));
}

$fragment = new rex_fragment();
$fragment->setVar('title', $t('image_sets_preview_title'), false);
$fragment->setVar('body', $previewBody, false);
echo $fragment->parse('core/page/section.php');

$jsI18n = json_encode([
    'optimal' => $t('image_sets_preview_verdict_optimal'), 'big' => $t('image_sets_preview_verdict_big'), 'small' => $t('image_sets_preview_verdict_small'), 'loading' => $t('image_sets_preview_loading'),
