...
foreach ($presets as $name => $config) {
    $form .= '<option value="' . rex_escape((string) $name) . '"' . ($name === $preset ? ' selected' : '') . '>' . rex_escape((string) $name) . ' (' . rex_escape((string) ($config['ratio'] ?? '')) . ')</option>';
}
$form .= '</select></div></div>';
...
$widths = $presetConfig['widths'] ?? [400, 800, 1200, 1600];
