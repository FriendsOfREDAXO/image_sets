...
namespace FriendsOfRedaxo\ImageSets\Media;
...
final class ResponsiveImage
{
     ...
     *
     * @param array{widths?: list<int>, sizes?: string} $options eigene Breitenstufen bzw. sizes fuer diese Quelle
     */
    public function withSource(string $media, string $preset, array $options = []): self
    {
        ...
        if (isset($options['widths']) && is_array($options['widths'])) {
            $widths = self::normalizeWidths($options['widths']);
            if ($widths === []) {
                $widths = null;
            }
        }
    ...
    public function getDimensions(string $preset = '', int $width = 0): array
    {
        ...
        if ($mode === 'focuspoint' && $ratio !== 'original' && class_exists('rex_effect_image_sets')) {
            [$rw, $rh] = \rex_effect_image_sets::resolveRatio($ratio);
            if ($rw > 0 && $rh > 0) {
    ...
    {
        return array_values(array_map(static fn (array $e) => $e[1], $this->getSrcsetEntries($preset)));
    }
    ...
    private function resolveSrc(string $preset): string
    {
        ...
        $entries = $this->densities !== [] ? $this->getDensityEntries($preset) : $this->getSrcsetEntries($preset);
        $typeWidth = $this->densities !== [] ? (int) $entries[0][0] : (int) end($entries)[0];
        if ($this->densities === []) {
            foreach ($entries as $entry) {
                if ($entry[0] === 1200) {
                    $typeWidth = 1200;
    ...
    /** @param list<int|string> $widths @return list<int> */
