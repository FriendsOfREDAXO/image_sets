...
final class PresetStore
{
    ...
    {
        return rex_addon::get('image_sets');
    }
     ...
     * @return array<string, array{ratio: string, mode: string, widths: list<int>, default_width: int, active: bool, note: string, updated: string}>
     */
    ...
    /** @return array{ratio: string, mode: string, widths: list<int>, default_width: int, active: bool, note: string, updated: string}|null */
    ...
    public static function save(string $name, array $input, string $originalName = ''): array
    {
        ...
        if (!preg_match('/^[a-z][a-z0-9_]{1,60}$/', $name)) {
            $errors[] = rex_i18n::msg('image_sets_msg_invalid_name');
        } elseif (self::isBuiltin($name)) {
            $errors[] = rex_i18n::msg('image_sets_msg_name_builtin');
        }
        ...
        if ($ratio !== 'original' && !preg_match('/^[1-9]\d{0,2}_[1-9]\d{0,2}$/', $ratio)) {
            $errors[] = rex_i18n::msg('image_sets_msg_invalid_ratio');
        }
        ...
        if ($widths === [] || count($widths) > self::MAX_STEPS) {
            $errors[] = rex_i18n::msg('image_sets_msg_invalid_widths', (string) self::MIN_WIDTH, (string) self::MAX_WIDTH, (string) self::MAX_STEPS);
        }
        ...
        if ($widths !== [] && !in_array($default, $widths, true)) {
            $errors[] = rex_i18n::msg('image_sets_msg_invalid_default');
        }
    ...
    /** @param array<string, mixed> $config @return array{ratio: string, mode: string, widths: list<int>, default_width: int, active: bool, note: string, updated: string}|null */
