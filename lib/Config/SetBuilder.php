...
final class SetBuilder
{
     ...
     * @return array{widths: list<int>, default_width: int, steps: list<array{label: string, css: int, x1: int, x2: int}>}
     */
    public static function suggest(array $o): array
    {
        ...
        $steps = [
            ['label' => 'phone', 'css' => (int) round(self::PHONE_WIDTH * $mobileVw / 100)],
            ['label' => 'mobile', 'css' => (int) round(($bpTablet - 1) * $mobileVw / 100)],
            ['label' => 'tablet', 'css' => (int) round(($bpDesktop - 1) * $tabletVw / 100)],
            ['label' => 'desktop', 'css' => $desktop],
        ];

        $candidates = [];
        foreach ($steps as &$step) {
            $step['x1'] = self::roundUp($step['css']);
            $step['x2'] = self::roundUp($step['css'] * 2);
            $candidates[] = $step['x1'];
            if ($retina) {
                $candidates[] = $step['x2'];
            }
